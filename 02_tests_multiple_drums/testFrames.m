...
%   Detailed explanation goes here
    ...
    'tests'
    if test==1
        'test quantile A with distance from min/max'
        ...
        for i=1:length(testdata(1,:))
            ...
            [ ResultsS ] = testQuantileS(Atmp, Stmp, SMean, SMin, SMax, windowSize, 1000);  
...
%               rA = ResultsA./mean(ResultsA);
%               rS = ResultsS./mean(ResultsS)./3;
%               
%               rGes = rA+rS;
%               
%             [minR,idx]=min(rGes);
...
%             fprintf('-----');
%             rACritS(i+1,1)=tidx;
%             rACritS(i+1,2:length(rACritS(1,:)))=rGes;
%             resultsS(tidx,idx)=resultsS(tidx,idx)+1;

        end
        dlmwrite(strcat('results\',timestamp,'_quantile_a_d_acrit_frames_A_matrix.csv'),rACritA,'delimiter','\t');
        dlmwrite(strcat('results\',timestamp,'_quantile_a_d_result_frames_A_matrix.csv'),resultsA,'delimiter','\t');
         dlmwrite(strcat('results\',timestamp,'_quantile_a_d_acrit_frames_S_matrix.csv'),rACritS,'delimiter','\t');
         dlmwrite(strcat('results\',timestamp,'_quantile_a_d_result_frames_S_matrix.csv'),resultsS,'delimiter','\t');
    end
